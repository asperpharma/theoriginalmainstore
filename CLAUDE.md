...
| Service | Value |
|---|---|
| Supabase Project | `mpcxpydkzvwlflxcujnz` |
| Supabase URL | `https://mpcxpydkzvwlflxcujnz.supabase.co` |
...
Quick reference for available skills (invoke with `/skill-name`):

| Skill | Purpose | Usage |
|---|---|---|
| `/deploy` | Deploy to Vercel/Supabase | `/deploy`, `/deploy functions`, `/deploy all` |
| `/deploy-function` | Deploy single edge function | `/deploy-function <name>` |
| `/sync-catalog` | Sync Shopify catalog | `/sync-catalog`, `/sync-catalog dry` |
| `/new-component` | Scaffold neumorphic component | `/new-component <ComponentName>` |
| `/workflow-init` | Scaffold a Vercel Workflow | `/workflow-init`, `/workflow-init <name>` |

See `AGENTS.md` for detailed skill documentation.

## MCP Setup

`.mcp.json` auto-loads three MCP servers when you open the project in Claude Code:

- **Supabase** — database + edge functions (read-only, scoped to this project)
- **Context7** — up-to-date library documentation lookup
- **Playwright** — browser automation for E2E work

Supabase requires a personal access token in your shell:

```bash
export SUPABASE_ACCESS_TOKEN="your-supabase-pat"
```

Context7 and Playwright need no extra setup — they launch via `npx` on demand.
...
| `slack-bot` | Slack workspace bot — orders, stats, AI concierge |

## Channel Webhooks
| Channel | Webhook URL |
|---|---|
| **Gorgias** | `https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/beauty-assistant?route=gorgias` |
| **ManyChat** (IG/FB/WA) | `https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/beauty-assistant?route=manychat` |
| **Telegram Bot** | `https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/telegram-bot` |
| **Slack Bot** | `https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/slack-bot` |
...
## Slack Commands
- `/orders` — today's orders summary
- `/stats` — sales + traffic stats
- `/search <query>` — product search
- `/brands` — top brands list
- `/help` — command reference
- Direct messages invoke AI concierge (Ms. Zain persona)

**Required Secrets:** `SLACK_BOT_TOKEN`, `SLACK_SIGNING_SECRET`, `SHOPIFY_WEBHOOK_SECRET`, `SLACK_ADMIN_CHANNEL`
