...
| `/deploy` | Instantly trigger deployment workflow (`/deploy`, `/deploy functions`, `/deploy telegram`, `/deploy all`). |
| `/deplo` / `/DEPLO` | Treat as shorthand typo for `/deploy` and execute deploy workflow immediately (case-insensitive). |
