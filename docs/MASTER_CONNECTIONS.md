...
**Domain registrar:** Cloudflare (previously GoDaddy)
**Hosting:** Lovable (custom domain verified via Lovable Settings > Domains)
**Important:** Cloudflare proxy must be set to "DNS only" (grey cloud) so Lovable can verify ownership and provision TLS certificates.
...
```
https://mpcxpydkzvwlflxcujnz.supabase.co
```
...
```
https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/{function_name}
```
...
All deployed at `https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/`
...
Base endpoint:
```
https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/beauty-assistant
```
...
|---|---|---|
| **Supabase** (DB + Auth + Edge Functions) | Active | `mpcxpydkzvwlflxcujnz` |
...
|---|---|---|
| Supabase Project ID | ~~`rgehleqcubtmcwyipyvi`~~ → ~~`vhgwvfedgfmcixhdyttt`~~ | `mpcxpydkzvwlflxcujnz` |
...
- [x] Cloudflare DNS: CONFIGURED (asperbeautyshop.com)
