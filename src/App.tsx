...
// Keyboard-only skip link: stays visually hidden until focused, then jumps to main content.
const SkipToContent = () => (
  <a
    href="#main-content"
    className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[10001] focus:px-4 focus:py-2 focus:rounded focus:bg-[#800020] focus:text-white focus:font-body focus:text-sm focus:tracking-wide focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#C5A028]"
  >
    Skip to main content
  </a>
);

// Per-route boundary: keyed off pathname so navigating away clears the error state.
function RouteBoundary({ children }: { children: React.ReactNode }) {
  const location = useLocation();
  return (
    <ErrorBoundary context="RouteBoundary" resetKey={location.pathname}>
      {children}
    </ErrorBoundary>
  );
}
      ...
      <LanguageProvider>
        <CartSyncProvider>
          <TooltipProvider>
            ...
            <BrowserRouter>
              <SkipToContent />
              {/* Floating assistants must never crash the page or block route render. */}
              <ErrorBoundary context="FloatingWidgets" fallback={() => null}>
              ...
              <div id="main-content" tabIndex={-1} className="outline-none">
                <RouteBoundary>
                  ...
                  <Route path="/admin/lab-tools" element={<RequireAdmin><LabTools /></RequireAdmin>} />
                  <Route path="/intelligence/chat" element={<Intelligence />} />
