...
const AMMAN_AREAS = [
  ...
  { en: "Marka", ar: "ماركا" },
  { en: "Zarqa Road", ar: "طريق الزرقاء" },
  { en: "Airport Road", ar: "طريق المطار" },
  { en: "Wadi Saqra", ar: "وادي صقرة" },
  { en: "Gardens (Hada'eq)", ar: "الحدائق" },
  { en: "Jubaiha", ar: "الجبيهة" },
...
export const CODCheckoutForm = (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/track-order?order=${result.orderNumber}&token=${result.token}`);
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div className="space-y-4">
        ...
        {formData.city === "Amman" && (
          <div className="space-y-2">
            <Label htmlFor="area" className="flex items-center gap-2 text-sm">
              <MapPin className="w-4 h-4 text-polished-gold" />
            ...
            >
              <SelectTrigger className="border-polished-gold/30 focus:border-polished-gold">
                <SelectValue
                  placeholder={isArabic ? "اختر المنطقة" : "Select area"}
                />
              </SelectTrigger>
              <SelectContent>
                {AMMAN_AREAS.map((a) => (
