...
const CelestialFeaturedCollection = () => {
  ...
  const fetchTray = useCallback(async (concern: SkinConcern) => {
    ...
    try {
      ...
      setTray(data as TrayResponse);
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : "Failed to load tray";
      console.error("Digital Tray fetch error:", errorMsg);
      setTray(null);
      toast.error(`Failed to load product recommendations: ${errorMsg}`);
    } finally {
      setIsLoading(false);
    }
  }, []);
        ...
        <div className="flex overflow-x-auto pb-2 scrollbar-hide md:flex-wrap justify-start md:justify-center gap-3 mb-12 -mx-6 px-6 md:mx-0 md:px-0" dir={isRTL ? "rtl" : "ltr"}>
          {CONCERNS.map((c) => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-body text-sm transition-all border ${activeConcern === c.tag
