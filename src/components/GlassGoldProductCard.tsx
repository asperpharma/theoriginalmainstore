...
export const GlassGoldProductCard = (
  ...
  return (
    <Link to={`/product/${node.handle}`} className="group block">
      {/* Digital Tray Card Container — Morning Spa */}
      <div className="relative w-full bg-polished-white border border-transparent rounded-sm overflow-hidden transition-all duration-500 hover:border-shiny-gold hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] hover:-translate-y-2">
        {/* Image Container */}
        <div className="relative aspect-[4/5] overflow-hidden">
            ...
            <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
              {isBestseller && (
                <div
                  ...
                  title="Bestseller"
                >
                  <Star className="w-4 h-4 text-polished-white fill-polished-white" />
                </div>
              )}
              {isNewArrival && !isBestseller && (
                <div
                  ...
                  title="New Arrival"
                >
                  <Sparkles className="w-4 h-4 text-polished-white" />
                </div>
              )}
              {isOnSale && (
                <div className="px-2 py-1 bg-burgundy text-polished-white font-sans text-xs tracking-wide rounded-full shadow-lg">
                  -{discountPercent}%
                </div>
              )}
            </div>
          ...
          <button
            ...
            className={`absolute top-3 right-3 z-20 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
              isWishlisted
                ? "bg-polished-gold text-polished-white"
                : "bg-polished-white/80 backdrop-blur-sm text-muted-foreground md:opacity-0 md:group-hover:opacity-100 hover:bg-polished-gold hover:text-polished-white"
            }`}
          >
            <Heart
          ...
          <div className="absolute bottom-0 left-0 w-full translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <button
              ...
              className="w-full bg-burgundy text-polished-white py-4 uppercase tracking-widest text-xs font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-colors"
            >
              <ShoppingBag size={16} />
        ...
        <div className="p-6 text-center">
          <p className="text-xs text-polished-gold uppercase tracking-[0.2em] mb-2">
            {brand}
          </p>
          <h3 className="font-display text-xl text-dark-charcoal mb-2 line-clamp-2">
