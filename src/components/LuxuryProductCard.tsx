  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    const cartProduct = {
      node: {
        ...
        compareAtPriceRange: {
          maxVariantPrice: {
            amount: (product.compare_at_price || price).toString(),
            currencyCode: "JOD",
          },
          minVariantPrice: {
            amount: (product.compare_at_price || price).toString(),
  ...
  return (
    <>
      <Link
        ...
        className="group relative bg-background border border-gold/10 rounded-xl flex flex-col h-full overflow-hidden transition-all duration-250 shadow-[6px_6px_14px_hsl(0_0%_85%/0.5),-6px_-6px_14px_hsl(0_0%_100%/0.7)] hover:shadow-[10px_10px_20px_hsl(0_0%_82%/0.6),-10px_-10px_20px_hsl(0_0%_100%/0.85)] hover:-translate-y-1 cursor-pointer"
      >
          ...
          <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/5 transition-colors duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full bg-cream/95 backdrop-blur-sm border border-gold/30 flex items-center justify-center cursor-pointer transition-all duration-200 shadow-[3px_3px_6px_rgba(0,0,0,0.1),-2px_-2px_4px_rgba(255,255,255,0.9)] hover:shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1),inset_-2px_-2px_4px_rgba(255,255,255,0.7)] hover:bg-gold hover:text-cream"
              ...
              <button
                ...
                className="hidden md:flex w-10 h-10 rounded-full bg-burgundy/95 backdrop-blur-sm border border-burgundy/50 items-center justify-center text-cream cursor-pointer transition-all duration-200 shadow-[3px_3px_6px_rgba(128,0,32,0.3),-2px_-2px_4px_rgba(255,255,255,0.1)] hover:shadow-[inset_2px_2px_4px_rgba(0,0,0,0.2),inset_-2px_-2px_4px_rgba(255,255,255,0.05)]"
            ...
            <div className="flex items-center gap-2">
              {product.compare_at_price && product.compare_at_price > price && (
                <span className="font-sans text-xs text-muted-foreground line-through">
                  {formatJOD(product.compare_at_price)}
                </span>
              )}
              <span
                className={`font-sans font-bold text-xs md:text-base ${
                  product.compare_at_price && product.compare_at_price > price ? "text-red-600" : "text-foreground"
                }`}
              >
                {formatJOD(price)}
              </span>
