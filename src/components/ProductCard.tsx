...
const getDNATag = (node: ShopifyProduct["node"]) => {
  const tags = Array.isArray(node.tags) ? node.tags.map(t => t.toLowerCase()) : [];
  const vendor = (node.vendor || "").toLowerCase();
  const title = (node.title || "").toLowerCase();
  // Check Supabase-style fields surfaced via metadata
  const meta = (node as { is_bestseller?: boolean; is_hero?: boolean; dr_sami_approved?: boolean }).valueOf ? node : {};
  if (tags.includes("best-seller") || tags.includes("bestseller") || (meta as { is_bestseller?: boolean }).is_bestseller)
    return { label: "Best Seller", bg: "#800020", color: "#fff" };
  if (tags.includes("organic"))
    return { label: "Organic", bg: "#2D6A4F", color: "#fff" };
  if (tags.includes("dermatologist") || vendor.includes("cerave") || vendor.includes("la roche") || vendor.includes("bioderma") || vendor.includes("vichy") || vendor.includes("eucerin"))
  ...
  if (vendor.includes("cerave") || vendor.includes("la roche-posay"))
    return { label: "Clinical Grade", bg: "#C5A028", color: "#fff" };
  return null;
};
  ...
  const handleWishlistToggle = (e: React.MouseEvent) => {
   ...
   return (
    <>
      ...
      >
        <div className={`relative bg-polished-white overflow-hidden transition-all duration-300 border border-transparent hover:border-shiny-gold p-5 ${isOutOfStock ? "opacity-60" : ""}`}>
          ...
          <div className="space-y-1.5">
            ...
            {/* Conversion CTA — visible always on touch, hover on desktop */}
            <div className={`pt-3 transition-all duration-300 md:${isHovered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
              <button
