...
import { CONCERN_CONFIG, normalizeConcernSlug } from "@/lib/concernMapping";
...
export function resolveCategoryQuery(categorySlug?: string): string | undefined {
  if (!categorySlug) return undefined;
  if (categorySlug === "best-sellers") return "best-sellers";

  const normalizedConcern = normalizeConcernSlug(categorySlug);
  if (!normalizedConcern) return undefined;
  const concernTag = CONCERN_CONFIG[normalizedConcern]?.tag;
  return concernTag ? `tag:${concernTag}` : undefined;
}
...
export const ProductGrid = ({
  ...
  const categoryQuery = useMemo(
    () => resolveCategoryQuery(categorySlug),
    [categorySlug],
  );
  ...
  }, [pageInfo, loadingMore, categoryQuery]);
  ...
  }, [products, categorySlug, categoryQuery]);
