...
export const ProductQuickViewModal = (
  ...
  const isOnSale = compareAt !== null && compareAt > product.price && product.price > 0;
  const discountPercent = isOnSale
    ? Math.round(((compareAt - product.price) / compareAt) * 100)
    : 0;

  const handleAddToCart = () => {
    logPrescriptionAddToCart(product.id, "quick_view");
    ...
    const cartProduct = {
      node: {
        ...
        compareAtPriceRange: {
          maxVariantPrice: {
            amount: (compareAt || product.price).toString(),
            currencyCode: "JOD",
          },
          minVariantPrice: {
            amount: (compareAt || product.price).toString(),
