            ...
            <div className="flex flex-wrap gap-2">
              {SKIN_CONCERNS.map((concern) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-body font-medium tracking-wider uppercase transition-all duration-200 rounded-none ${
                    filters.skinConcerns.includes(concern.id)
                      ? "bg-burgundy text-polished-white border border-burgundy"
                      : "bg-transparent border border-polished-gold/40 text-asper-ink/60 hover:border-polished-gold hover:text-asper-ink"
                  }`}
                >
              ...
              <div className="flex flex-wrap gap-2">
                {PRICE_RANGES.map((range) => (
                  <button
                    ...
                    onClick={() =>
                      updateFilters({
                        priceRange: [range.min, Math.min(range.max, maxPrice)],
                      })}
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {filters.categories.map((catId) => {
            ...
            return (
              <Badge
                key={catId}
                variant="secondary"
                className="bg-burgundy/10 text-burgundy border border-burgundy/30 hover:bg-burgundy hover:text-polished-white cursor-pointer gap-1 rounded-none transition-all duration-200"
          ...
          {filters.subcategories.map((subId) => {
            const subcategory = CATEGORIES.flatMap((category) => category.subcategories)
            ...
            return (
              <Badge
                ...
                onClick={() => toggleArrayFilter("subcategories", subId)}
              >
                {language === "ar" ? subcategory?.labelAr : subcategory?.labelEn}
                <X className="w-3 h-3" />
              </Badge>
            );
          })}
