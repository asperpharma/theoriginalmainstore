...
export const RoutineSaver = ({ products, concern }: RoutineSaverProps) => {
  ...
  const handleSaveToProfile = async () => {
    ...
    try {
      ...
      toast.success(
        language === "ar" 
          ? "تم حفظ الروتين في ملفك الشخصي" 
          : "Routine saved to your profile"
      );
    } catch (error) {
      console.error("Save error:", error);
      toast.error(
        language === "ar" ? "فشل الحفظ" : "Failed to save routine"
      );
    } finally {
      setIsSaving(false);
    }
    ...
    return (
      <div className="space-y-3 p-3 bg-asper-stone/30 rounded-lg border border-shiny-gold/20 animate-in fade-in slide-in-from-top-2">
        <p className="text-xs text-asper-ink font-body">
          {language === "ar" 
            ? "أدخل بريدك لحفظ الروتين والوصول إليه لاحقاً" 
        ...
        <div className="flex gap-2">
          ...
          >
            {isSaving ? <Loader2 className="w-3 h-3 animate-spin" /> : (language === "ar" ? "حفظ" : "Save")}
          </Button>
        </div>
        <button 
    ...
    >
      {isSaving ? (
        ...
        <>
          ...
          {language === "ar" ? "حفظ الروتين في ملفي" : "Save Routine to Profile"}
        </>
      )}
    </Button>
  );
