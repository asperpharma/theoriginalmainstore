...
export function AsperExperience() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isVisible) return;
      // Ignore events that originate from editable elements to avoid
      // unexpected carousel navigation while the user is typing.
  ...
  return (
    <section ref={sectionRef} className="w-full bg-background py-16 md:py-24">
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        ...
        >
          <h3 className={cn(
            "font-display text-2xl md:text-3xl text-accent mb-2",
            isAr && "font-arabic"
          )}>
            {isAr ? "قوتنا الخارقة" : "Our Superpower"}
          </h3>
          <p className={cn(
            "font-body text-base md:text-lg text-muted-foreground max-w-2xl mx-auto",
            isAr && "font-arabic"
          )}>
            {isAr
              ? "نحن نمزج بين الدقة السريرية والفخامة الاستثنائية لنقدم لكِ نتائج ملموسة وتجربة لا تُنسى."
              : "We blend clinical precision with exceptional luxury to deliver real results and an unforgettable experience. This is our superpower—your beauty, redefined."}
          </p>
        </motion.div>
          ...
          <div className="relative w-full aspect-video">
            {isVisible &&
              VIDEOS.map(({ src }, i) => (
                <video
                  ...
                  className={cn(
                    "absolute inset-0 w-full h-full object-cover object-[75%_50%] md:object-center transition-opacity duration-1200 ease-luxury-out",
              ...
              <div className="flex justify-center">
                <Link
                  ...
                  className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/30
                             ...
                             hover:bg-white/25 transition-all duration-400 min-h-[44px]"
                >
            ...
            <button
              ...
              className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 items-center justify-center
                         ...
                         hover:bg-background/40 transition-all duration-300 rounded-full focus:outline-none focus:ring-2 focus:ring-accent"
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex gap-3">
            {VIDEOS.map((_, i) => (
              <button
                ...
                className={cn(
                  "h-3 rounded-full transition-all duration-400 min-h-[12px] min-w-[12px] focus:outline-none focus:ring-2 focus:ring-accent",
                  i === currentIdx
                    ? "bg-accent w-8"
