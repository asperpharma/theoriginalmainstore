...
export default function ConciergeShowcase() {
  ...
  const typedText = useTypingEffect(
    isAr ? "أريد بشرة مشرقة، لكنني حامل." : "I want a glow, but I'm pregnant.",
    45,
    400,
    phase === "typing"
  );
  ...
  return (
    <section ref={ref} className="py-20 sm:py-28 bg-background overflow-hidden" dir={isRTL ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <Badge variant="outline" className="mb-4 border-primary/30 text-primary font-body text-xs tracking-[0.2em] px-4 py-1.5">
            <Shield className="h-3 w-3 mr-2" />
            {isAr ? "قفل الأمان · شخصيتان مزدوجتان" : "SAFETY INTERLOCK · DUAL PERSONA"}
          </Badge>
          <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground">
            {isAr ? <>ذكاء <span className="text-primary">يحميكِ</span></> : <>Intelligence That <span className="text-primary">Protects</span></>}
          </h2>
          <p className="mt-4 text-muted-foreground text-lg max-w-2xl mx-auto font-body">
            {isAr ? "يكتشف ذكاؤنا الاصطناعي تلقائياً التعارضات ويستبدل المكونات غير الآمنة — للحفاظ على إشراقك بأمان." : "Our AI automatically detects contraindications and swaps unsafe ingredients — keeping your glow safe."}
          </p>
        </div>
          ...
          <div className="rounded-lg border border-border bg-card shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-2 px-5 py-3 border-b border-border bg-muted/30">
              ...
              <span className="ml-2 text-xs font-body text-muted-foreground tracking-wider uppercase">
                {isAr ? "مساعد الذكاء الاصطناعي" : "AI Concierge"}
              </span>
            </div>
                  ...
                  >
                    ...
                    <div className="text-xs font-body">
                      <p className="font-semibold text-foreground">
                        {phase === "interlock"
                          ? (isAr ? "⚠️ تفعّل قفل الأمان" : "⚠️ Safety Interlock Triggered")
                          : (isAr ? "✅ اجتاز فحص الأمان" : "✅ Safety Check Passed")}
                      </p>
                      <p className="text-muted-foreground mt-0.5">
                        {phase === "interlock"
                          ? (isAr ? "تم تحديد الريتينول — موانع استخدام أثناء الحمل." : "Retinol flagged — contraindicated during pregnancy.")
                          : (isAr ? "تمت المراجعة. جميع المكونات آمنة أثناء الحمل." : "Regimen cleared. All ingredients pregnancy-safe.")}
                      </p>
                    </div>
                  </motion.div>
                )}
                  ...
                  >
                    ...
                    <div className="bg-muted rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%]">
                      ...
                      <p className="font-body text-sm text-foreground">
                        {isAr
                          ? <>{`فحص الأمان: `}<span className="line-through text-muted-foreground">ريتينول</span>{` تمت إزالته. استُبدل بفيتامين C — فعّال بنفس القدر للإشراق، وآمن تماماً أثناء الحمل.`}</>
                          : <>Safety Check: <span className="line-through text-muted-foreground">Retinol</span> removed. Replaced with Vitamin C — equally effective for radiance, fully pregnancy-safe.</>}
                      </p>
                    </div>
                  </motion.div>
                )}
                ...
                {phase === "regimen" && (
                  ...
                  >
                    ...
                    <div className="bg-muted rounded-2xl rounded-bl-md px-4 py-2.5 max-w-[85%]">
                      ...
                      <p className="font-body text-sm text-foreground">
                        {isAr ? "تم فتح الإشراق بأمان. ✨ روتين توهجك جاهز!" : "Radiance unlocked safely. ✨ Your glow regimen is ready!"}
                      </p>
                    </div>
                  </motion.div>
                )}
          ...
          <div className="rounded-lg border border-border bg-card shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-muted/30">
              <span className="text-xs font-body text-muted-foreground tracking-wider uppercase">
                {isAr ? "روتينك" : "Your Regimen"}
              </span>
              <Badge variant="secondary" className="text-[10px] font-body">
                {isAr ? "حل بـ3 نقرات" : "3-Click Solution"}
              </Badge>
            </div>
                  ...
                  >
                    ...
                    <div>
                      <p className="font-body text-sm font-semibold text-foreground line-through">
                        {isAr ? "سيروم ريتينول الليلي" : "Retinol Night Serum"}
                      </p>
                      <p className="font-body text-xs text-destructive">
                        {isAr ? "⚠ موانع استخدام — الحمل" : "⚠ Contraindicated — Pregnancy"}
                      </p>
                    </div>
                  </motion.div>
                )}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="font-body text-xs text-green-700">
                        {isAr ? "✓ بديل آمن أثناء الحمل" : "✓ Pregnancy-safe alternative"}
                      </p>
                    </div>
                  </motion.div>
                )}
                    ...
                    >
                      <Link to="/intelligence">
                        <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-11 text-sm uppercase tracking-widest shadow-lg shadow-primary/20 group">
                          ...
                          {isAr ? "حل بـ3 نقرات" : "The 3-Click Solution"}
