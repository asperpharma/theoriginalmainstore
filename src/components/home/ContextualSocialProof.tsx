...
function ContextTag({ label, value }: { label: string; value: string }) {
  return (
    <span className="neu-flat inline-block px-2.5 py-1 bg-card border border-border text-muted-foreground text-xs font-body tracking-wide">
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="section-header text-3xl md:text-4xl font-display text-foreground mb-4">
            {isAr
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {REVIEWS.map((review) => (
            <article
              key={review.id}
              className="neu-raised bg-background border border-border p-8 flex flex-col transition-all duration-[500ms] ease-[cubic-bezier(0.19,1,0.22,1)] hover:-translate-y-1"
            >
              {/* Stars & Verification */}
