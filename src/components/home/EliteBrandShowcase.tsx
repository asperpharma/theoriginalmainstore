    ...
    <section className="bg-asper-stone-light py-24 px-6 md:px-12 lg:px-24 w-full">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="section-header text-3xl md:text-5xl font-display text-burgundy tracking-wide uppercase mb-4">
            Curated Excellence
          </h2>
          <div className="gold-accent-line mx-auto w-16" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-16">
          {brands.map((brand) => (
            <Link to={`/brands/${brand.slug || brand.id}`} key={brand.id} className="group block cursor-pointer">
              <div className="neu-raised relative overflow-hidden w-full aspect-[4/5] bg-white transition-all duration-700 ease-in-out group-hover:shadow-2xl group-hover:ring-1 group-hover:ring-polished-gold group-hover:ring-offset-4 group-hover:ring-offset-asper-stone-light">
                <img
