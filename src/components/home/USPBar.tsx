...
export const USPBar = () => {
  ...
  return (
    <section className="py-8 bg-burgundy relative">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-12 lg:gap-20">
          {USP_ITEMS.map((item, index) => (
            <div key={item.en} className="flex items-center gap-4">
              <div className="neu-raised w-12 h-12 rounded-full bg-burgundy flex items-center justify-center flex-shrink-0 border border-polished-gold/20">
                <item.icon className="w-5 h-5 text-polished-gold" />
              </div>
              <div className={isArabic ? "text-right" : "text-left"}>
                <p className={`font-display text-sm text-polished-white font-semibold leading-tight${isArabic ? " font-arabic" : ""}`}>
                  {isArabic ? item.ar : item.en}
                </p>
                <p className={`font-body text-[11px] text-polished-gold/60${isArabic ? " font-arabic" : ""}`}>
