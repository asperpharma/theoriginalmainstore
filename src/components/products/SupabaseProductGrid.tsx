...
function EliteProductCard({ product, index, featured = false }: { product: Product; index: number; featured?: boolean }) {
  ...
  return (
    ...
    >
          ...
          {imageUrl ? (
            <>
              ...
              <BlurUpImage
                ...
                priority={index < 4}
                className={cn(
                  "w-full h-full object-contain mix-blend-multiply transition-all duration-700 ease-luxury-out",
                  hoverImageUrl
      ...
      <div
        className="hidden md:block absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-luxury-out bg-primary text-primary-foreground py-3.5 text-center cursor-pointer uppercase text-[10px] font-bold tracking-[0.2em] hover:bg-primary/90 active:scale-[0.98]"
        onClick={handleAddToCart}
      >
