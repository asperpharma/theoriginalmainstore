...
export const SKIN_CONCERNS: SkinConcern[] = [
  ...
  },
  {
    ...
    labelAr: "إصلاح الحاجز",
    color: "bg-amber-100 text-amber-700",
  },
];

const SKIN_CONCERN_DB_MAP: Record<string, string[]> = {
  acne: ["Concern_Acne"],
  "anti-aging": ["Concern_Aging", "Concern_AntiAging"],
  hydration: ["Concern_Hydration", "Concern_Dryness"],
  "oily-skin": ["Concern_Oiliness"],
  "dry-skin": ["Concern_Dryness"],
  sensitivity: ["Concern_Sensitivity", "Concern_Redness"],
  "dark-spots": ["Concern_Brightening", "Concern_Pigmentation"],
  wrinkles: ["Concern_Aging", "Concern_AntiAging"],
  "sun-protection": ["Concern_SunProtection"],
  redness: ["Concern_Redness", "Concern_Sensitivity"],
  cleansing: ["Concern_Acne"],
  "pregnancy-safe": ["Concern_PregnancySafe"],
  "rosacea-safe": ["Concern_Rosacea", "Concern_Redness", "Concern_Sensitivity"],
  "glass-skin": ["Concern_Brightening", "Concern_Hydration"],
  "barrier-repair": ["Concern_Dryness", "Concern_Sensitivity", "Concern_Hydration"],
  brightening: ["Concern_Brightening", "Concern_Pigmentation"],
  "dark-circles": ["Concern_DarkCircles"],
};

const DB_TO_FILTER_CONCERN_MAP: Record<string, string> = {
  Concern_Acne: "acne",
  Concern_Aging: "anti-aging",
  Concern_AntiAging: "anti-aging",
  Concern_Hydration: "hydration",
  Concern_Dryness: "dry-skin",
  Concern_Sensitivity: "sensitivity",
  Concern_Redness: "redness",
  Concern_Brightening: "brightening",
  Concern_Pigmentation: "dark-spots",
  Concern_SunProtection: "sun-protection",
  Concern_DarkCircles: "dark-circles",
  Concern_Oiliness: "oily-skin",
  Concern_PregnancySafe: "pregnancy-safe",
  Concern_Rosacea: "rosacea-safe",
};
...
export const normalizeConcernFilterId = (id: string): string => {
  return DB_TO_FILTER_CONCERN_MAP[id] || id;
};

export const mapSkinConcernToDbConcerns = (concernId: string): string[] => {
  if (concernId.startsWith("Concern_")) return [concernId];
  return SKIN_CONCERN_DB_MAP[concernId] || [];
};

/**
 * Maps a UI category ID (from CATEGORIES) or a primary_pillar value to its
 * primary_concern tags for database filtering.
 * Covers both the CATEGORIES id format (kebab-case) and the ASPER_CATEGORIES
 * primary_pillar values (exact DB strings).
 */
export const mapCategoryToConcerns = (categoryId: string): string[] => {
  const map: Record<string, string[]> = {
    // UI category IDs (kebab-case from CATEGORIES)
    'skin-care':    ['Concern_Acne', 'Concern_AntiAging', 'Concern_Hydration', 'Concern_Sensitivity', 'Concern_Pigmentation', 'Concern_Dryness', 'Concern_SunProtection', 'Concern_Brightening', 'Concern_RosaceaSafe', 'Concern_BarrierRepair'],
    'makeup':       ['Concern_Brightening', 'Concern_Oiliness', 'Concern_Pigmentation'],
    'hair-care':    ['Concern_HairLoss'],
    'body-care':    ['Concern_Dryness', 'Concern_Sensitivity'],
    'fragrance':    [],

    // primary_pillar DB values (exact strings stored in products.primary_pillar)
    'Skincare':               ['Concern_Acne', 'Concern_AntiAging', 'Concern_Hydration', 'Concern_Sensitivity', 'Concern_Pigmentation', 'Concern_Dryness', 'Concern_SunProtection', 'Concern_Brightening', 'Concern_RosaceaSafe', 'Concern_BarrierRepair'],
    'Beauty & Cosmetics':     ['Concern_Brightening', 'Concern_Oiliness', 'Concern_Pigmentation'],
    'Haircare':               ['Concern_HairLoss'],
    'Baby & Mom':             ['Concern_PregnancySafe', 'Concern_Sensitivity'],
    'Personal Care':          ['Concern_Dryness', 'Concern_Sensitivity'],
