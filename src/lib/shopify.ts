...
function rowToProduct(row: DbRow): ShopifyProduct {
    ...
    const inStock =
          ...
          (row.inventory_total != null && row.inventory_total > 0);
    return {
          node: {
                  id: row.id,
                  title,
...
export async function fetchProducts(
  ...
  const { tag, categorySlug } = options;
  const filters = query ? query.split(/\s+AND\s+/i).map((part) => part.trim()).filter(Boolean) : [];
  ...
  const applyQueryFilters = (filterParts: string[]) => {
    let builder = q;
    for (const part of filterParts) {
      if (part.startsWith("tag:")) {
        const tagVal = part.slice(4).trim();
        if (tagVal) builder = builder.contains("tags", [tagVal]);
      } else if (part === "best-sellers") {
        builder = builder.or("is_bestseller.eq.true,bestseller_rank.not.is.null");
      } else if (part.startsWith("vendor:")) {
        const vendor = part.slice(7).trim();
        if (vendor) builder = builder.ilike("brand", vendor);
      }
    }
    return builder;
  };
  ...
  if (filters.length > 0) {
    q = applyQueryFilters(filters);
  }

  const tagFilter = tag ?? categorySlug;
  if (tagFilter) {
...
export async function fetchProductsPaginated(
  ...
  if (filters.length > 0) {
    for (const part of filters) {
      if (part.startsWith("tag:")) {
        const tagVal = part.slice(4).trim();
        if (tagVal) q = q.contains("tags", [tagVal]);
      } else if (part === "best-sellers") {
        q = q.or("is_bestseller.eq.true,bestseller_rank.not.is.null");
      } else if (part.startsWith("vendor:")) {
        const vendor = part.slice(7).trim();
        if (vendor) q = q.ilike("brand", vendor);
      }
    }
  }
  ...
  if (error) {
      ...
      const sanitized = searchTerm.replace(/[%_,()[\]]/g, " ").trim();
        ...
        const { data: fallback, error: fallbackError } = await supabase
          ...
          .select(PRODUCT_COLUMNS)
          .or(
                    `title.ilike.${term},name.ilike.${term},description.ilike.${term},brand.ilike.${term}`,
                  )
          .limit(first);
