...
export async function searchProducts(
  ...
  if (error) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- backend row shape
    return (fallback ?? []).map((row: unknown) => rowToProduct(row as any));
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- backend row shape
  return (data ?? []).map((row: unknown) => rowToProduct(row as any));
}
