...
export default function ConcernCollection() {
  ...
  usePageMeta({
    title: title
      ? `${title} | ${language === "ar" ? "أسبر بيوتي شوب" : "Asper Beauty Shop"}`
      : (language === "ar" ? "مجموعة المنتجات | أسبر بيوتي شوب" : "Product Collection | Asper Beauty Shop"),
    description: config
      ? (language === "ar" ? `اكتشف أفضل منتجات ${title} المختارة بعناية من خبراء أسبر` : `Discover the best ${title} products curated by Asper's experts.`)
      : undefined,
    canonical: concernSlug ? `/concerns/${concernSlug}` : "/skin-concerns",
  });
    ...
    const load = async () => {
      try {
        ...
        if (normalized) {
          ...
          const filtered = filterProductsByConcern(normalized_products, normalized);
            ...
            <div className="space-y-8">
              <div className="text-center py-6">
                <p className="font-body text-gray-500 text-lg">
                  {language === "ar"
                    ? "لم يتم العثور على منتجات مطابقة. إليك المجموعة الكاملة للتصفح."
                    : "No exact concern matches found. Browse the full collection instead."}
                </p>
                <Link
                  to="/skin-concerns"
