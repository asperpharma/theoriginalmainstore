  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <div className="grid lg:grid-cols-2 min-h-screen pt-20 md:pt-28">
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="grid lg:grid-cols-2 min-h-screen pt-20 md:pt-28">
        ...
        <div className="lg:sticky lg:top-28 lg:h-[calc(100vh-7rem)] lg:overflow-y-auto bg-background">
          <div className="p-8 lg:p-16 flex flex-col justify-center min-h-full">
            ...
            {/* Key Clinical Actives — animated stagger section */}
            {product.key_ingredients && product.key_ingredients.length > 0 && (
              <div className="mb-8">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground mb-4">
                  {isArabic ? "المكونات الفعالة الرئيسية" : "Key Clinical Actives"}
                </p>
                <motion.div
                  initial="hidden"
                  whileInView="visible"
                ...
                >
                  {product.key_ingredients.map((ing, index) => (
                    <motion.div
                      key={`${ing}-${index}`}
                      variants={{
                        hidden: { opacity: 0, ...(prefersReducedMotion ? {} : { y: 30, scale: 0.98 }) },
                        visible: {
                          opacity: 1,
                      ...
                      className="clinical-glass px-3 py-1.5 rounded-full bg-accent/10 border border-accent/30 text-xs text-foreground font-medium"
                    >
                      {ing}
                    </motion.div>
                  ))}
