...
export default function Profile() {
  ...
  usePageMeta({
    title: isArabic ? "ملفي الشخصي | أسبر بيوتي شوب" : "My Profile | Asper Beauty Shop",
    description: isArabic ? "إدارة ملفك الشخصي وتفضيلاتك في أسبر بيوتي شوب" : "Manage your profile and preferences at Asper Beauty Shop.",
    canonical: "/profile",
  });
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      if (concierge) {
        ...
        setRoutine(concierge.recommended_routine as RegimenProfile);
      }
      ...
      if (recentConsultations && recentConsultations.length > 0) {
        setLedgerEntries(
          recentConsultations.map((c) => {
            const regimen = c.regimen as RegimenProfile | null;
            const title = regimen?.title || (c.channel === "whatsapp" ? "WhatsApp Consultation" : "Clinical Consultation");
            const persona = regimen?.persona || (c.channel === "whatsapp" ? "ms_zain" : "dr_sami");
