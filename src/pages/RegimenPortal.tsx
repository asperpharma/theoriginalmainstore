...
function DecryptionScreen({ isAr }: { isAr: boolean }) {
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6">
      ...
      <p className="font-body text-xs tracking-[0.2em] text-primary uppercase font-bold text-center animate-pulse">
        {isAr ? "فك تشفير السجل" : "Decrypting Ledger"}
        <br />
        {isAr ? "إنشاء رابط آمن" : "Establishing Secure Link"}
      </p>
    </div>
  );
}
...
function StepCard({
  ...
  const label = (isAr ? STEP_LABELS_AR : STEP_LABELS_EN)[step.step] || step.step;
  const instruction =
    step.instruction ||
    (step.step === "cleanser"
      ? (isAr ? "دلّكي بلطف لمدة 60 ثانية بالماء الفاتر." : "Massage gently for 60 seconds with lukewarm water.")
      : step.step === "treatment"
        ? (isAr ? "ضعي نقطتين على البشرة الرطبة قليلاً لحبس الترطيب." : "Apply 2 drops to slightly damp skin to lock in moisture.")
        : (isAr ? "ضعيه بسخاء كآخر خطوة، وأعيدي التطبيق كل ساعتين تحت الشمس." : "Apply generously as last step, reapply every 2 hours in sun."));
...
export default function RegimenPortal() {
  ...
  usePageMeta({
    title: isAr ? "بروتوكول العناية | أسبر بيوتي شوب" : "Regimen Protocol | Asper Beauty Shop",
    description: isAr
      ? "بروتوكول العناية بالبشرة المخصص لكِ من فريق أسبر الصيدلاني."
      : "Your pharmacist-prescribed skincare regimen — curated by Asper's clinical team.",
    canonical: "/regimen",
  });
      ...
      try {
        ...
        if (plan) {
          ...
          setPortalData({
            ...
            steps: regimenSteps,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- plan shape from backend
  ...
  if (isDecrypting) return <DecryptionScreen isAr={isAr} />;
  ...
  return (
    <div className="min-h-screen bg-background max-w-md mx-auto shadow-2xl relative pb-36" dir={isRTL ? "rtl" : "ltr"}>
      {/* Header */}
      <header className="p-6 border-b border-polished-gold/20 bg-card">
        ...
        <div className="bg-foreground p-4 rounded-lg">
          <p className="font-body text-[10px] text-polished-gold tracking-widest uppercase mb-1">
            {isAr ? "البروتوكول المستهدف" : "Target Protocol"}
          </p>
          <p className="font-heading text-lg text-primary-foreground">
            {isLoading
              ? (isAr ? "جاري التحميل..." : "Loading...")
              : portalData?.protocolName || (isAr ? "بروتوكول سريري" : "Clinical Protocol")}
          </p>
          <div className="flex justify-between items-center mt-3 border-t border-muted-foreground/30 pt-3">
            <span className="font-body text-[10px] text-muted-foreground/70">
              {isAr ? `وصفه ${portalData?.prescribedBy || "د. سامي"}` : `Prescribed by ${portalData?.prescribedBy || "Dr. Sami"}`}
            </span>
            <span className="font-body text-[10px] text-muted-foreground/70">
      ...
      <main className="p-6">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-heading text-lg text-foreground">
              {isAr ? "روتين الصباح" : "Morning Ritual"}
            </h2>
          </div>

          <div className={`relative border-polished-gold/30 ${isRTL ? "border-r mr-2" : "border-l ml-2"} space-y-6`}>
            {portalData?.steps.map((step, i) => (
              <StepCard key={step.product?.id || i} index={i} step={step} isAr={isAr} />
            ))}
          </div>
        </div>
        ...
        {portalData?.clinicalNote && (
          <div className="bg-card border border-polished-gold/20 p-4 rounded-lg shadow-sm">
            ...
            <p className={`font-body text-[10px] text-primary font-bold uppercase mt-2 tracking-widest ${isRTL ? "text-left" : "text-right"}`}>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-card border-t border-border p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50">
        <div className="flex justify-between items-end mb-3 px-2">
          <span className="font-body text-xs text-muted-foreground uppercase tracking-widest">
            {isAr ? "إجمالي البروتوكول" : "Protocol Total"}
          </span>
        ...
        >
          ...
          {isAr ? "تنفيذ البروتوكول بأمان" : "Securely Fulfill Regimen"}
        </Button>
      </div>
    </div>
  );
