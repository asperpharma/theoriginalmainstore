...
// Type definitions for Supabase RPC responses
interface RegimentPlanRow {
  id: string;
  title: string;
    ...
    async function fetchRegimen() {
      ...
      try {
        ...
        if (plan) {
          ...
          if (tray) {
            const trayData = tray as TrayResponse;
            const steps: RegimenStep[] = [];
            
            const stepKeys: Array<keyof TrayResponse> = ["step_1", "step_2", "step_3"];
            const stepLabels = ["cleanser", "treatment", "protection"];
            
            for (let i = 0; i < stepKeys.length; i++) {
              const product = trayData[stepKeys[i]];
              if (product) {
                steps.push({
                  step: stepLabels[i],
                  product: {
