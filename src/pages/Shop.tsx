...
import {
  CATEGORIES,
  getCategoryById,
  mapCategoryToConcerns,
  mapSkinConcernToDbConcerns,
  normalizeConcernFilterId,
...
const ShopProductCard = ({
  ...
  if (viewMode === "list") {
    return (
      <article
        className="group bg-card rounded-xl overflow-hidden border border-gold/10 transition-all duration-250 cursor-pointer flex shadow-[6px_6px_14px_hsl(0_0%_85%/0.5),-6px_-6px_14px_hsl(0_0%_100%/0.7)] hover:shadow-[10px_10px_20px_hsl(0_0%_82%/0.6),-10px_-10px_20px_hsl(0_0%_100%/0.85)] hover:-translate-y-1"
  ...
  return (
    <article
      className="group relative bg-card rounded-xl overflow-hidden border border-gold/10 transition-all duration-250 cursor-pointer flex flex-col animate-fade-in shadow-[6px_6px_14px_hsl(0_0%_85%/0.5),-6px_-6px_14px_hsl(0_0%_100%/0.7)] hover:shadow-[10px_10px_20px_hsl(0_0%_82%/0.6),-10px_-10px_20px_hsl(0_0%_100%/0.85)] hover:-translate-y-1"
      onClick={() => onQuickView(product)}
    >
      ...
      <div className="p-4 flex flex-col flex-grow space-y-2">
        ...
        <Button
          ...
          size="sm"
          className="w-full bg-burgundy hover:bg-burgundy-light text-white text-xs uppercase tracking-wide rounded-lg cursor-pointer transition-all duration-200 shadow-[4px_4px_8px_hsl(345_100%_25%/0.3),-2px_-2px_6px_hsl(0_0%_100%/0.6)] hover:shadow-[6px_6px_12px_hsl(345_100%_25%/0.35),-3px_-3px_8px_hsl(0_0%_100%/0.7)] active:shadow-[inset_3px_3px_6px_hsl(345_100%_18%/0.4)]"
        >
...
const AMBITION_PILLS = [
  { id: "acne", icon: "\u{1F48A}", labelEn: "Acne Care", labelAr: "\u0639\u0644\u0627\u062C \u062D\u0628 \u0627\u0644\u0634\u0628\u0627\u0628" },
  { id: "anti-aging", icon: "\u23F0", labelEn: "Anti-Aging", labelAr: "\u0645\u0643\u0627\u0641\u062D\u0629 \u0627\u0644\u0634\u064A\u062E\u0648\u062E\u0629" },
  { id: "hydration", icon: "\u{1F4A7}", labelEn: "Hydration", labelAr: "\u062A\u0631\u0637\u064A\u0628" },
  { id: "sensitivity", icon: "\u{1F33F}", labelEn: "Sensitive Skin", labelAr: "\u0628\u0634\u0631\u0629 \u062D\u0633\u0627\u0633\u0629" },
  { id: "dark-spots", icon: "\u{1F31F}", labelEn: "Dark Spots", labelAr: "\u0627\u0644\u0628\u0642\u0639 \u0627\u0644\u062F\u0627\u0643\u0646\u0629" },
  { id: "sun-protection", icon: "\u2600\uFE0F", labelEn: "Sun Protection", labelAr: "\u062D\u0645\u0627\u064A\u0629 \u0645\u0646 \u0627\u0644\u0634\u0645\u0633" },
  { id: "brightening", icon: "\u2728", labelEn: "Brightening", labelAr: "\u0625\u0634\u0631\u0627\u0642\u0629" },
  { id: "dry-skin", icon: "\u{1F6E1}\uFE0F", labelEn: "Dryness", labelAr: "\u062C\u0641\u0627\u0641" },
];
...
const buildMatchTokens = (values: Array<string | null | undefined>) => {
...
const buildDefaultFilters = (maxPrice: number, initialConcern?: string): FilterState => ({
  searchQuery: "",
  categories: [],
  subcategories: [],
  brands: [],
  skinConcerns: initialConcern ? [normalizeConcernFilterId(initialConcern)] : [],
  priceRange: [0, maxPrice],
  onSaleOnly: false,
});

const getSubcategoryTokens = (subcategoryId: string) => {
  const matchedSubcategory = CATEGORIES.flatMap((category) => category.subcategories)
    .find((subcategory) => subcategory.id === subcategoryId);

  return buildMatchTokens(matchedSubcategory
    ? [matchedSubcategory.id, matchedSubcategory.labelEn, matchedSubcategory.labelAr]
    : [subcategoryId]);
};

const getRecommendedSortScore = (product: Product) => {
  const freshnessScore = product.created_at ? new Date(product.created_at).getTime() / 1_000_000_000_000 : 0;
  const inventoryScore = product.in_stock === false ? 0 : 5_000;
  const bestsellerScore = product.is_bestseller ? 800 : 0;
  const heroScore = product.is_hero ? 200 : 0;
  const saleScore = product.is_on_sale ? 300 + (product.discount_percent ?? 0) * 10 : 0;
  const rankScore = product.bestseller_rank ? Math.max(0, 100 - product.bestseller_rank) : 0;
  return inventoryScore + bestsellerScore + heroScore + saleScore + rankScore + freshnessScore;
};
...
export default function Shop() {
  ...
  const [filters, setFilters] = useState<FilterState>(() => buildDefaultFilters(200, concernParam));
  ...
  const maxPrice = useMemo(() => {
    const highestPrice = products.reduce((highest, product) => Math.max(highest, product.price ?? 0), 0);
    return Math.max(200, Math.ceil(highestPrice / 5) * 5);
  }, [products]);
  ...
  useEffect(() => {
    ...
    setFilters((current) => {
      if (!priceRangeInitializedRef.current) {
        ...
        return { ...current, priceRange: [0, maxPrice] };
      }

      const nextMin = Math.min(current.priceRange[0], maxPrice);
      const nextMax = Math.max(nextMin, Math.min(current.priceRange[1], maxPrice));
      if (nextMin === current.priceRange[0] && nextMax === current.priceRange[1]) {
        return current;
      }

      return { ...current, priceRange: [nextMin, nextMax] };
    });
  ...
  useEffect(() => {
    ...
    const normalizedConcern = normalizeConcernFilterId(concernParam);
    setFilters((current) => (
      current.skinConcerns.includes(normalizedConcern)
        ? current
        : { ...current, skinConcerns: [...current.skinConcerns, normalizedConcern] }
    ));
  }, [concernParam]);
  ...
  const clearAllFilters = () => {
    ...
    next.delete("concern");
    setSearchParams(next, { replace: true });
    setFilters(buildDefaultFilters(maxPrice));
  };
  ...
  const filteredProducts = useMemo(() => {
    const activeConcernFilters = new Set(filters.skinConcerns.flatMap(mapSkinConcernToDbConcerns));
    ...
    const filtered = products.filter((product) => {
      ...
      if (
        ...
        (!product.brand || !filters.brands.some((brand) => normalizeMatchValue(brand) === normalizeMatchValue(product.brand)))
      ) {
        return false;
      }

      const productTokens = new Set(buildMatchTokens([
        product.category,
        product.asper_category,
        product.primary_concern,
        ...(product.tags ?? []),
      ...
      if (filters.categories.length > 0) {
        const matchesCategory = filters.categories.some((categoryId) => {
          ...
          const categoryTokens = buildMatchTokens(category
            ? [category.id, category.labelEn, category.labelAr]
            : [categoryId]);
          return (
            categoryTokens.some((token) => productTokens.has(token)) ||
            (!!product.primary_concern && mapCategoryToConcerns(categoryId).includes(product.primary_concern))
          );
        });
      ...
      if (filters.subcategories.length > 0) {
        const matchesSubcategory = filters.subcategories.some((subcategoryId) => {
          return getSubcategoryTokens(subcategoryId).some((token) => productTokens.has(token));
        });
        if (!matchesSubcategory) return false;
      }

      if (activeConcernFilters.size > 0 && (!product.primary_concern || !activeConcernFilters.has(product.primary_concern))) {
        return false;
      }
    ...
    if (sortBy === "recommended") {
      sorted.sort((a, b) => getRecommendedSortScore(b) - getRecommendedSortScore(a));
        ...
        <>
          ...
          <div className="flex gap-2 justify-center">
            <Button
              ...
              onClick={() => { setActiveCategory("Clinical Serums & Actives"); setFilters((current) => ({ ...current, searchQuery: "" })); }}
            >
              Clinical Serums
            </Button>
            <Button
