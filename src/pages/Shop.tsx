...
import type { FilterState } from "@/components/ProductSearchFilters";
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-background flex items-center justify-center p-4">
        ...
        {!(product.compare_at_price && product.compare_at_price > (product.price ?? 0)) && product.clinical_badge && (
...
const ASPER_CATEGORIES = [
  { key: "All Curation",           en: "All Products",          ar: "جميع المنتجات" },
  { key: "Skincare",               en: "Skincare",               ar: "العناية بالبشرة" },
  { key: "Beauty & Cosmetics",     en: "Beauty & Cosmetics",     ar: "الجمال ومستحضرات التجميل" },
  { key: "Haircare",               en: "Haircare",               ar: "العناية بالشعر" },
  { key: "Baby & Mom",             en: "Baby & Mom",             ar: "الأم والطفل" },
  { key: "Personal Care",          en: "Personal Care",          ar: "العناية الشخصية" },
  { key: "Supplements & Vitamins", en: "Supplements & Vitamins", ar: "المكملات والفيتامينات" },
  { key: "Pharmacy & Wellness",    en: "Pharmacy & Wellness",    ar: "الصيدلية والعافية" },
  { key: "Oral Care",              en: "Oral Care",              ar: "العناية بالفم" },
] as const;
...
const CategorySidebar = ({
  ...
  <div className="sticky top-24 space-y-1">
    ...
    {ASPER_CATEGORIES.map(({ key, en, ar }) => {
      const isActive = activeCategory === key;
      const count = key === "All Curation" ? undefined : categoryCounts[key];
      return (
        <button
          key={key}
          onClick={() => onSelect(key)}
          className={cn(
            "w-full px-3 py-2 text-sm font-body transition-all duration-300 flex items-center justify-between",
            isAr ? "text-right border-r-2" : "text-left border-l-2",
            isActive
              ? cn("border-accent text-foreground font-semibold", isAr ? "pr-4" : "pl-4")
              : cn("border-transparent text-muted-foreground hover:text-foreground hover:border-accent/40", isAr ? "hover:pr-4" : "hover:pl-4")
          )}
...
export default function Shop() {
  ...
  usePageMeta({
    ...
    description: isAr
      ? "تسوقي من أكثر من 6,500 منتج للعناية بالبشرة والشعر والجمال، مختارة بعناية من صيادلة متخصصين في الأردن."
      : "Shop 6,500+ premium skincare, haircare and beauty products, curated by pharmacist experts in Jordan.",
    canonical: "/shop",
  });
  ...
  // ?section=X maps to primary_pillar — set by CategoryIcons nav bar
  const sectionParam = searchParams.get("section") ?? "";

  // Active section from sidebar (mirrors ?section=X set by nav bar)
  const categoryParam = sectionParam || "All Curation";
  const setActiveCategory = (cat: string) => {
    const next = new URLSearchParams(searchParams);
    if (cat === "All Curation") {
      next.delete("section");
    } else {
      next.set("section", cat);
    }
  ...
  // Reset to page 0 whenever section, concern filter, or sort changes
  useEffect(() => {
    setPage(0);
    setProducts([]);
    setHasMore(true);
  }, [sectionParam, filters.skinConcerns, sortBy]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Sort at DB level for correct pagination across all pages
        // Note: api.products view columns — is_on_sale/discount_percent do NOT exist;
        // use is_bestseller, compare_at_price, bestseller_rank instead
        if (sortBy === "recommended") {
          // Best-sellers first (lower rank = more popular), nulls last; then hero products
          query = query
          ...
          // Products with a compare_at_price (marked down) first, then by bestseller_rank
          query = query
        ...
        // Filter by section at DB level
        if (sectionParam) {
          query = query.eq("primary_pillar", sectionParam);
        }

        // Filter by concern at DB level — concern pills must query the full dataset, not just current page
        if (filters.skinConcerns.length > 0) {
          query = query.in("primary_concern", filters.skinConcerns);
        }
    ...
    fetchProducts();
  }, [page, sectionParam, filters.skinConcerns, sortBy]);

  // Fetch total counts per pillar from DB (full dataset, not just current page)
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        for (const { primary_pillar } of data) {
          if (primary_pillar) counts[primary_pillar] = (counts[primary_pillar] || 0) + 1;
        }
  ...
  const filteredProducts = useMemo(() => {
    const filtered = products.filter((product) => {
      ...
      if (filters.onSaleOnly && !(product.compare_at_price && product.compare_at_price > (product.price ?? 0))) return false;
      return true;
    });

    // Sort is handled at DB level for correct pagination — preserve DB order here
    return filtered;
  }, [products, filters, sectionParam]);
        ...
        <>
          ...
          <div className="flex gap-2 justify-center">
            <Button
              ...
              onClick={() => { setActiveCategory("Skincare"); setFilters(f => ({ ...f, searchQuery: "" })); }}
            >
              Clinical Serums
            </Button>
            <Button
                ...
                >
                  {ASPER_CATEGORIES.map(({ key, en, ar }) => {
                    ...
                    return (
                      ...
                      >
                        <span className="whitespace-nowrap">{isAr ? ar : en}</span>
                        {count !== undefined && (
                  ...
                  {hasMore && (
                    <div className="flex justify-center pt-10">
                      <button
                        ...
                        disabled={isLoading}
                        className="px-8 py-3 bg-burgundy text-polished-white font-body font-medium text-sm uppercase tracking-widest hover:bg-burgundy-light transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        {isLoading
                          ? (isAr ? "جارٍ التحميل..." : "Loading...")
                          : (isAr ? "تحميل المزيد من المنتجات" : "Load More Products")}
                      </button>
                    </div>
                  )}
                </>
