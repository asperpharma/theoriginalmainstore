  ...
  try {
      ...
      if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({
            ...
            reply: "Intelligence protocol reset required. Please standby."
          }),
    ...
    if (action === "tts") {
      ...
      if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({ error: "TTS service temporarily unavailable. Please try again." }),
          { status: 503, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      ...
      if (!part?.inlineData) {
        console.error("No audio data in Gemini TTS response:", data);
        return new Response(
