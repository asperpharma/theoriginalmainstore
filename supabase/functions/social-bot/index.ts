 ...
 *
 * Webhook:
 *   https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://vhgwvfedgfmcixhdyttt.supabase.co/functions/v1/social-bot
 *
 * Required secrets:
...
async function cmdSchedule(chatId: string, args: string) {
  ...
  const scheduledAt = new Date(`${dateStr}T${timeStr}:00+03:00`);
  if (isNaN(scheduledAt.getTime())) return send(chatId, "❌ Invalid date/time. Use format: YYYY-MM-DD HH:MM (Amman time)");
  ...
  await send(chatId,
    ...
    `📝 ${esc(content.slice(0, 100))}${content.length > 100 ? "…" : ""}\n` +
    `🆔 <code>${data.id}</code>`
  );
