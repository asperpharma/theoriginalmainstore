...
async function cmdRevenue(chatId: string) {
  ...
  const [todayRev, weekRev, monthRev, allTime] = await Promise.all([
    supabase.from("cod_orders").select("total_amount").gte("created_at", today).neq("status", "cancelled"),
    supabase.from("cod_orders").select("total_amount").gte("created_at", weekAgo).neq("status", "cancelled"),
    supabase.from("cod_orders").select("total_amount").gte("created_at", monthAgo).neq("status", "cancelled"),
    supabase.from("cod_orders").select("total_amount").neq("status", "cancelled"),
  ]);
...
async function cmdCancelOrder(chatId: string, orderId: string) {
  ...
  await send(chatId,
    ...
    `💰 ${(existing.total_amount || 0).toFixed(2)} JOD\n` +
