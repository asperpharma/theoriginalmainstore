 ...
 *
 * Webhook:
 *   https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://mpcxpydkzvwlflxcujnz.supabase.co/functions/v1/telegram-bot
 *
 * Required secrets:
...
const SOCIAL_LINKS = {
  ...
  facebook:  "https://www.facebook.com/AsperBeautyShop",
...
SOCIAL NETWORK (always redirect users here when they ask to follow/contact/find us):
...
- Tell users they can tap /social to open any of these directly in the bot.
...
async function cmdSocial(chatId: string) {
  const msg =
    `${BRAND.DIVIDER}\n` +
    `◈ <b>ASPER BEAUTY SHOP</b> · Social Network ◈\n` +
    `${BRAND.DIVIDER}\n\n` +
    `Follow us for launches, rituals &amp; clinical insights:\n\n` +
    `🌐 <a href="${SOCIAL_LINKS.website}">asperbeautyshop.com</a>\n` +
    `💬 <a href="${SOCIAL_LINKS.whatsapp}">WhatsApp · +962 79 065 6666</a>\n\n` +
    `<i>Tap a button below to open directly.</i>`;
  await send(chatId, msg, {
    reply_markup: {
      inline_keyboard: [
        [
        ...
        [
          { text: "👻 Snapchat",  url: SOCIAL_LINKS.snapchat },
          { text: "𝕏 Twitter",    url: SOCIAL_LINKS.x },
        ],
        [
...
async function cmdBulkPrice(chatId: string, args: string) {
  ...
  if (fetchError) return send(chatId, `❌ ${esc(fetchError.message)}`);
...
async function cmdTopSellers(chatId: string) {
  ...
  for (const order of data) {
    ...
    for (const item of items as Array<{ title?: string; name?: string; quantity?: number }>) {
      const title = item.title || item.name || "Unknown";
      counts[title] = (counts[title] || 0) + (item.quantity || 1);
    }
  }
  const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 10);
  if (!sorted.length) return send(chatId, "📊 No product data in the last 30 days.");

  let msg = `${BRAND.DIVIDER}\n◈ <b>TOP SELLERS</b> (30 days) ◈\n${BRAND.DIVIDER}\n\n`;
  sorted.forEach(([title, count], i) => {
    msg += `<b>${i + 1}.</b> ${esc(title)} — ${count} sold\n`;
  });
  msg += BRAND.FOOTER;
  await send(chatId, msg);
}
...
async function cmdExportProducts(chatId: string, args: string) {
  ...
  const csvEsc = (v: string | null | undefined): string => {
    const s = v ?? "";
    return `"${s.replace(/"/g, '""')}"`;
  };
...
function getHelp(admin: boolean): string {
  ...
  msg += `📱 <b>Connect</b>\n`;
  msg += `  /social — Our Instagram, TikTok, WhatsApp &amp; more\n\n`;
...
Deno.serve(async (req) => {
  ...
  switch (cmd) {
    ...
    case "/social": await cmdSocial(chatId); return new Response("OK");
